...
const pusher = new Pusher('2843f5e60fbb0548ebb7', {
    cluster: 'us3'
  });
...
const Chat = () => {
    ...
    const retrieveConversation = (channelId) => {
        if(channelId){
    ...
    useEffect(() => {
        retrieveConversation(channelId)
    ...
    return (
        <div className='chat' >
            ...
            <div className="chat__messages">
                ...
                {messages.map(message => (
                    <Message key={message.id} message={message.message} timestamp={message.timestamp} user={message.user} />
                ))}
            </div>
